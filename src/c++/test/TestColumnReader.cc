...
std::unique_ptr<ColumnReader> buildInvalidUnionTagReader(
    MockStripeStreams& streams, bool hasNulls = false) {
  // only the struct and the union itself are selected, so the union
  // children never need their own streams
  ...
  if (hasNulls) {
    EXPECT_CALL(streams, getStreamProxy(1, proto::Stream_Kind_PRESENT, true))
        .WillRepeatedly(testing::Return(new SeekableArrayInputStream
                                        (ONE_PRESENT_VALUE,
                                         ARRAY_SIZE(ONE_PRESENT_VALUE))));
  }

  EXPECT_CALL(streams, getStreamProxy(1, proto::Stream_Kind_DATA, true))
      .WillRepeatedly(testing::Return(new SeekableArrayInputStream
                                      (INVALID_UNION_TAG,
                                       ARRAY_SIZE(INVALID_UNION_TAG))));
...
TEST(TestColumnReader, testUnionEncodedRejectsInvalidTag) {
  MockStripeStreams streams;
  std::unique_ptr<ColumnReader> reader = buildInvalidUnionTagReader(streams);

  StructVectorBatch batch(1, *getDefaultPool());
  addSingleUnionBatch(batch);
  EXPECT_THROW(reader->nextEncoded(batch, 1, 0), ParseError);
}

TEST(TestColumnReader, testUnionRejectsInvalidTagWithNulls) {
  MockStripeStreams streams;
  std::unique_ptr<ColumnReader> reader =
      buildInvalidUnionTagReader(streams, true);
